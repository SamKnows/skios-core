...
#ifdef DEBUG
//  int buff_size = 0;
//  int err = 0, len = 0, trysize = 0, gotsize = 0;
//  
//  len = sizeof(buff_size);
//  err = getsockopt(CFSocketGetNative(theSocket4),SOL_SOCKET,SO_SNDBUF,(char*)&buff_size,&len);
//  NSLog(@"DEBUG: IP4 Size was %d\n",buff_size);
//  len = sizeof(int);
//  trysize = 10485760+32768;
//  do {
//    trysize -= 32768;
//    setsockopt(CFSocketGetNative(theSocket4),SOL_SOCKET,SO_SNDBUF,(char*)&trysize,len);
//    len = sizeof(buff_size);
//    err = getsockopt(CFSocketGetNative(theSocket4),SOL_SOCKET,SO_SNDBUF,(char*)&gotsize,&len);
//    if (err < 0) { perror("getsockopt"); break; }
//  } while (gotsize < trysize);
//  printf("IP4 Size set to %d\n",gotsize);
  //err = getsockopt(theSocket6,SOL_SOCKET,SO_SNDBUF,(char*)&size,&len);
  
//  len = sizeof(buff_size);
//  err = getsockopt(CFSocketGetNative(theSocket6),SOL_SOCKET,SO_SNDBUF,(char*)&buff_size,&len);
//  NSLog(@"DEBUG: IP6 Size was %d\n",buff_size);
//  len = sizeof(int);
//  trysize = 10485760+32768;
//  do {
//    trysize -= 32768;
//    setsockopt(CFSocketGetNative(theSocket6),SOL_SOCKET,SO_SNDBUF,(char*)&trysize,len);
//    len = sizeof(buff_size);
//    err = getsockopt(CFSocketGetNative(theSocket6),SOL_SOCKET,SO_SNDBUF,(char*)&gotsize,&len);
//    if (err < 0) { perror("getsockopt"); break; }
//  } while (gotsize < trysize);
//  printf("IP6 Size set to %d\n",gotsize);
#endif // DEBUG
             ...
             tag:(long)tag
{
	...
	NSData *address4 = nil, *address6 = nil;
	int res = [self convertForSendHost:host port:port intoAddress4:&address4 address6:&address6];
  if (res != 0) {
    return NO;
  }
	...
	if (theCurrentReceive != nil)
	{
		...
		do
		{
			...
			if([self hasBytesAvailable:theSocket])
			{
              ...
              // Take ownership of buf (and set our alloced buf to NULL, to prevent it getting free-d twice!)
							theCurrentReceive->buffer = [[NSData alloc] initWithBytesNoCopy:buf length:result freeWhenDone:YES];
              buf = NULL;
              ...
              buf = NULL;
				...
				// If the buffer is not NULL, this means it has not been taken over.
				if (buf != NULL)
				{
					free(buf);
          buf = NULL;
