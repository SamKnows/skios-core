...
- (instancetype)initWithDirection:(NSString*)direction Parameters:(NSArray*)params
{
  ...
  if (self) {
    ...
    [self sLatestSpeedReset:(downstream ? cReasonResetDownload : cReasonResetUpload)];
...
-(BOOL)  isSuccessful {
  return [super.testStatus isEqualToString:@"OK"];
...
-(void) execute {													/* Execute test */
  ...
  for (int i = 0; i < nThreads; i++) {
    [threads  addObject:[NSThread alloc] init];
  }
